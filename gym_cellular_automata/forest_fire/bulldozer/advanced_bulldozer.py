...
class AdvancedForestFireBulldozerEnv(CAEnv):
    ...
    def _award(self, prev_grid, grid, per_env_context):
        ...
        cell_counts = jnp.array(
            [dict_counts[self._empty], dict_counts[self._tree], dict_counts[self._fire], dict_counts[self._bulldozed]]
        )

        cell_counts_relative = cell_counts / ncells

        reward_weights = jnp.array(
            [self._reward_per_empty, self._reward_per_tree, self._reward_per_fire, self._reward_per_bulldozed]
        )
